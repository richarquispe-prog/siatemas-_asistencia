...
if(mysqli_num_rows($sql1)> 0){
	...
	//para marcar salida
	if($marca=='S'){
		$update = mysqli_query($conexion, "UPDATE marcas SET hora_s='$hora' WHERE codigo='$codigo' AND fecha='$fecha'") or die(mysqli_error());
		$update1 = mysqli_query($conexion, "UPDATE `estudiantes` SET `disponible` = '0' WHERE `codigo` = '$codigo'");
